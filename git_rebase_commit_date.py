...
def run_git_command(repo_path, command, background=False, shell=True):
    ...
    if not background:
        ...
        return subprocess.Popen(command, cwd=repo_path, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                                text=True, shell=shell)


def wait_for_file(filepath, timeout=10, interval=1):
...
def status(repo_path):
    command = ["git", "status"]
    run_git_command(repo_path, command, shell=False)

def start_interactive_rebase(repo_path, commit_hash):
    """Starts an interactive rebase from the commit's parent."""
    rebase_command = ["git", "rebase", "-i", f"{commit_hash}^"]
    run_git_command(repo_path, rebase_command, background=True, shell=False)


def modify_rebase_todo_file(repo_path, commit_hash):
    """Modifies the rebase todo file to set the commit for editing."""
    rebase_todo_path = os.path.join(repo_path, ".git", "rebase-merge", "git-rebase-todo")
    if not wait_for_file(rebase_todo_path):
...
def amend_commit_date(repo_path, new_date):
    """Amends the commit with the new date."""
    amend_command = f'GIT_COMMITTER_DATE="{new_date}" GIT_AUTHOR_DATE="{new_date}" git commit --amend --no-edit'
    run_git_command(repo_path, amend_command, shell=True)


def continue_rebase(repo_path):
    """Continues the rebase process."""
    continue_rebase_command = ["git", "rebase", "--continue"]
    run_git_command(repo_path, continue_rebase_command, shell=False)
...
def git_rebase_commit_date(repo_path, commit_hash, new_date):
    ...
    status(repo_path)
    start_interactive_rebase(repo_path, commit_hash)
    if modify_rebase_todo_file(repo_path, commit_hash):
        amend_commit_date(repo_path, new_date)
        continue_rebase(repo_path)
